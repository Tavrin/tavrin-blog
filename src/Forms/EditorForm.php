...
class EditorForm extends Form
{
    public function __construct(Request $request, object $entity, Session $session, array $options = [])
    {
        ...
        $this->addCss('w-75 w-90-xs')
            ...
            ->addDateTimeInput('createdAt', ['class' => 'form-control', 'placeholder' => "Date de publication", 'value' => 'edit' === $options['type']? $entity->getCreatedAt()->format("Y-m-d\TH:i:s") : date("Y-m-d\TH:i:s")])
            ->addDateTimeInput('updatedAt', ['class' => 'form-control', 'placeholder' => "Date de modification", 'required' => false, 'modifyIfEmpty' => false, 'value' => 'edit' === $options['type']? $entityUpdated : null])
